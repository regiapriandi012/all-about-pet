...
<form action="{{ route('update.tipsmerawatkandang',['id' => $tipsmerawatkandang->id]) }}" method="POST" enctype="multipart/form-data">
@method('PATCH')    
@csrf
<h3>Update Artikel Tips Merawat Kandang</h3>
    <fieldset>

        <p style="text-align: left;"><label for="title">Artikel Title</label></p>
        <input style="  width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 4px !important; resize: vertical;" type="text" id="title" name="title" value="{{ old('title') ?? $tipsmerawatkandang->title }}"> @error('title')
        <div class="text-danger">{{ $message }}</div> @enderror

        <p style="text-align: left;"><label for="author">Author</label></p>
        <input style="  width: 100%; padding: 12px; border: 1px solid #ccc; border-radius: 4px !important; resize: vertical;" type="text"  id="author" name="author" value="{{ old('author') ?? $tipsmerawatkandang->author }}"> @error('author')
        <div class="text-danger">{{ $message }}</div> @enderror

        <p style="text-align: left;"><label for="content">Content</label></p>
        <textarea style="  width: 100%; height: 150px; padding: 12px 20px; box-sizing: border-box; border: 2px solid #ccc; border-radius: 4px !important; background-color: #f8f8f8; font-size: 16px; resize: none;" id="content" rows="10" cols="130" name="content">{{ old('content') ?? $tipsmerawatkandang->content}}</textarea>

        <p style="text-align: left;"><label for="image">Image</label></p>
        <input style="  width: 100%; padding: 5px; border: 1px solid #ccc; border-radius: 4px !important; resize: vertical;" type="file" id="image" name="image" value="{{ old('image') ?? $tipsmerawatkandang->image }}"> @error('image')
        <div class="text-danger">{{ $message }}</div> @enderror
